...
angular.module('epsonreceipts.images').directive('imageViewer', function($q, $timeout, $sce) {
  return {
    restrict: 'E',
    scope: {
      image: '='
    ...
    link: function($scope, $element, $attributes) {
      ...
      $scope.$watch(function() { return $element.width(); }, resize);
      $scope.$watch(function() { return $element.height(); }, resize);
      ...
      $scope.$watch('image', function(image_) {
        ...
        if (image_) {
          $scope.message = $sce.trustAsHtml('Loading<br><i class="fa fa-spin fa-spinner"></i>');
        } else {
          $scope.message = $sce.trustAsHtml('No image');
        }
