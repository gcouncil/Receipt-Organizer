...
angular.module('epsonreceipts.itemsToolbar').directive('itemsToolbarFolderButton', function(
  folderStorage,
  $templateCache,
  $dropdown,
  $q,
    ...
    link: function($scope, $element) {
      ...
      dropdown.$scope.folderItems = function(folder) {
        ...
        $q.all(promises).then(function() {
          if (counter === 0) {
            notify.error('Selected ' + pluralize('item', items.length) +
                         ' already in ' + folder.name);
          } else {
            notify.success('Added ' + counter + pluralize(' item', counter) +
                           ' to ' + folder.name);
          }
        }, function() {
