...
angular.module('epsonreceipts.items').directive('itemsCollectionScope', function(
  $state,
  $stateParams,
  $controller,
  itemStorage,
      ...
      this.setFilter = function(type, filter) {
        ...
        if (type === 'category') {
          ...
          query.setFilter('startDate', function(item) {
            ...
            console.log('start', filter, date >= moment(filter));
          ...
          query.setFilter('endDate', function(item) {
            if (!item.date) { return false; }
            var date = moment(item.date);
            console.log('end', filter, date <= moment(filter));
            var momentFilter = moment(filter);
            return (date <= momentFilter);
          });
        }
