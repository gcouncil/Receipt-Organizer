...
angular.module('epsonreceipts.reports.report-editor').controller('ReportEditorController', function(
  ...
  function recalculate() {
    ...
    var summary = { since: null, until: null, total: 0, totalRequested: 0, count: 0 };
    ...
    _.each($scope.items, function(item) {
      ...
      if (_.isFinite(item.totalRequested)) {
        summary.totalRequested += item.totalRequested;
      }
  ...
  $scope.remove = function(items) {
    var ids = _.isArray(items) ? _.pluck(items, 'id') : [items.id];
