...
angular.module('epsonreceipts.reports.report-export').controller('ReportExportController', function(
  ...
  $scope.downloadSupported = typeof document.createElement('a').download !== 'undefined';

  $scope.isOffline = function() {
    return offlineStorage.isOffline();
  };
  ...
  $scope.isExporterDisabled = function(extension) {
    return offlineStorage.isOffline() && !$scope.downloadSupported && extension !== 'pdf';
  };
  ...
  $scope.generate = function() {
    ...
    var options = {
      ...
      // IIF
      creditAccount: $scope.settings.creditAccount,
      debitAccount: $scope.settings.debitAccount,
    };
    ...
    $q.all(promises).then(function(items) {
      ...
      exprt.exporter($scope.report, items, options, services, function(err, stream) {
        ...
        output.on('finish', function() {
          ...
          } else if (!$scope.downloadSupported) {
            var reader = new FileReader();
            reader.onload = function() {
