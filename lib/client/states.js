...
angular.module('epsonreceipts').config(function($stateProvider, $urlRouterProvider) {
  ...
  $stateProvider.state('toplevel', {
    abstract: true,
    template: '<toplevel-layout items-collection-scope></toplevel-layout>',
  });
  ...
  $stateProvider.state('reports', {
    ...
    views: {
      ...
      '': {
        template: [
          '<ng-switch on="$state.params.view" class="flex-container">',
          '<reports-list-view ng-switch-when="list" items="items.pagination.items" selection="items.selection"></reports-list-view>',
          '<reports-thumbnail-grid ng-switch-default items="items.pagination.items" selection="items.selection" pagination="items.pagination"></reports-thumbnail-grid>',
          '</ng-switch>'
        ].join('')
      }
    }
