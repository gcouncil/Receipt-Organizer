...
angular.module('epsonreceipts.storage').factory('folderStorage', function(domain, $http, $q) {
  ...
  return {
    ...
    create: function(folder) {
      return $http.post('/api/folders', folder).then(function(result) {
        notify();
        return new domain.Folder(result.data);
      });
    },

    update: function(folder) {
      return $http.put('/api/folders/' + folder.id, folder).then(function(result) {
        notify();
        return new domain.Folder(result.data);
      });
    },
    ...
    destroy: function(folder) {
      return $http.delete('/api/folders/' + folder.id, folder).then(function(result) {
        notify();
      });
    }
  };
