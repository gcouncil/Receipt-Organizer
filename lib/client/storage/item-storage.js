...
angular.module('epsonreceipts.storage').factory('itemStorage', function(
  ...
  function ItemQuery($scope, callback) {
    var legalSorts = ['vendor', 'total', 'category', 'folders', 'date', 'type'];

    var sort = sort || function(item) {
    ...
    var folders;
    folderStorage.query({}).then(function(results) {
      folders = results;
    });

    this.setSort = function(type, reverse_) {
      if (!_.contains(legalSorts, type)) {
        return;
      }
      ...
      if (type === 'date') {
        ...
        sort = function(item) {
          var itemFolders = _.filter(folders, function(folder) {
            return _.contains(item.folders, folder.id);
          });

          var folderNames = _.map(itemFolders, function(folder) {
            return folder.name;
          });
