...
angular.module('epsonreceipts.storage').factory('reportStorage', function(
  ...
  var reportStorage = {
    sync: function() {
      var dfd = $q.defer();
      var promises;
      // New local report, post to server, remove from local cache and indexdb store
      offlineStorage.load('reports').then(function(reports) {
        promises = _.chain(reports)
          .map(function(report) {
            return [
              $http.post('/api/reports', report),
              offlineStorage.store('reports', report.id, undefined)
            ];
          })
          .flatten()
          .value();
