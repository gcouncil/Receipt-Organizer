...
angular.module('epsonreceipts.users.signup-form').directive('signupForm', function() {
  return {
    ...
    controller: function($scope, $state, userStorage, flashManager) {
      $scope.submit = function() {
        userStorage.create($scope.user).then(function() {
          flashManager.addMessage('Successfully signed up!', 'success');
          $state.go('receipts.thumbnails');
        }, function(response) {
          flashManager.addMessage(response.data.message, 'danger');
        });
      };
    }
  };
