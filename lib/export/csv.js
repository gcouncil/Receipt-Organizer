...
module.exports = function(report, items, options, services, callback) {
  ...
  var csvHeaders = processCSVHeaders(services.getUser(), csvItems[0]);
...
function processCSVHeaders(user, item){
  // add in nice Names
  var fieldList = _.map(user.settings.fields, function(field){  return field.name });
