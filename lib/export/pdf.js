...
function makeEntries(items) {
  ...
  _.each(items, function(item, i) {
    if ((item.receipt || item.id) !== receipt) {
      ...
      var images = _.select(item.images || []);

      if (images.length > 0) {
        var imageEntries = _.map(images, function(image, page) {
          var entry = {
            type: 'image',
            page: page + 1,
            image: image
        ...
        imageEntry = imageEntries[0];
        imageEntry.first = i + 1;
      } else {
        imageEntry = {};
      }
    }
  ...
  async.series([
    function(callback) {
      async.eachLimit(ctx.items, 4, function(item, callback) {
        async.eachSeries(item.images || [], function(imageId, callback) {
          if (!imageId || ctx.images.hasOwnProperty(imageId)) {
            return callback(null);
          }

          services.getImage(imageId, function(err, image) {
            ctx.images[imageId] = image;
            callback(err);
          });
        }, callback);
      }, callback);
