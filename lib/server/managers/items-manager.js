...
module.exports = function(connection) {
  ...
  function authorize(id, user, callback) {
    ...
    fetch(query, function(err, objects) {
      ...
      if (object.user !== user && user !== true) { return callback(Boom.forbidden()); }
  ...
  var ItemsManager = {};
  _.extend(ItemsManager, require('events').EventEmitter.prototype);
  ...
  ItemsManager.deleteFolders = function(id, options, callback) {
    var query = table
    .where(table.folders.contains(sql.array(id).cast('uuid[]')))
    .toQuery();
