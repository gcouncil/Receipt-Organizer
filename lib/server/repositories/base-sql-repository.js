...
BaseSqlRepository.prototype._insert = function(row, callback) {
  ...
  self._query(query, function(err, results) {
    ...
    if (results.rows[0].id) {
      self._emit('created', results.rows[0].id, done);
    } else {
      done();
    }
...
BaseSqlRepository.prototype._update = function(row, callback) {
  ...
  self._query(query, function(err, results) {
    ...
    if (results.rows[0].id) {
      self._emit('updated', results.rows[0].id, done);
    } else {
      done();
    }
...
BaseSqlRepository.prototype._emit = function(name, data, callback) {
  if (!this.events) { return callback(); }
  this.events.emit(this.namespace + ':' + name, data, callback);
};
...
BaseSqlRepository.prototype.destroy = function(object, callback) {
  ...
  async.series([
    ...
    function(callback) {
      self._emit('destroyed', object.id, callback);
    }
  ], callback);
};
