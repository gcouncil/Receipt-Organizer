...
module.exports = function(app, handlers, config) {
  ...
  app.post('/roundtrip', function(req, res, next) {
    var match = req.body.data.match(/^data:([^;]+);[^,]+,(.*)$/);

    var type = match[1];
    var data = new Buffer(match[2], 'base64');
  ...
  app.put('/users/:user/settings', auth, handlers.users.updateSettings);
  app.put('/users/:user', auth, handlers.users.update);
};
