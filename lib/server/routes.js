...
module.exports = function(app, handlers, config) {
  ...
  var auth = passport.authenticate('token', { session: false });
  ...
  app.get('/images/:image/display', auth, handlers.images.display);
  app.post('/images', auth, handlers.images.create);
